...
async function addRematch(channel, resultMsg, user1, user2, commandName) {
  // Safely add rematch button — never throws
  try {
    ...
    // If edit fails just continue silently
    return;
  }
  ...
  try {
    const btnCol = await resultMsg.awaitMessageComponent({
      filter: function(b) {
        return [user1.id, user2.id].includes(b.user.id) &&
               b.customId.startsWith('rematch_' + commandName);
      },
      time: 30000
    });
    ...
    await btnCol.reply({
      content: '<@' + other.id + '> — **' + challenger.username + '** wants a rematch! Type `accept` or `decline`'
    });
    try {
      const rCol = await channel.awaitMessages({
        filter: function(m) {
          return m.author.id === other.id &&
                 ['accept','decline'].includes(m.content.toLowerCase().trim());
        },
      ...
      if (accepted) {
        await channel.send({ content: '🔁 Rematch accepted! Use `/' + commandName + '` to run it back.' });
      }
    } catch(e) {
      await resultMsg.edit({ components: [] }).catch(function() {});
    }
