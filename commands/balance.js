...
module.exports = {
  data: new SlashCommandBuilder().setName('balance').setDescription('💰  Check the Konvault LTC wallet'),
  async execute(interaction){
    ...
    try{
      ...
      const lines=['`'+address+'`','','**LTC Price:** `$'+ltcPrice.toFixed(2)+'`','','💰  **Confirmed:** `'+confirmed.toFixed(8)+' LTC`  •  `'+toUSD(confirmed)+'`','⏳  **Pending:** `'+unconfirmed.toFixed(8)+' LTC`  •  `'+toUSD(unconfirmed)+'`','📥  **Received:** `'+totalRecv.toFixed(8)+' LTC`  •  `'+toUSD(totalRecv)+'`','📤  **Sent:** `'+totalSent.toFixed(8)+' LTC`  •  `'+toUSD(totalSent)+'`','🔁  **Transactions:** `'+balData.n_tx+'`'];
      if(txLines.length)lines.push('','**Recent Incoming**',...txLines);
      lines.push('','[View on Blockchain](https://live.blockcypher.com/ltc/address/'+address+'/)');
      await interaction.editReply({embeds:[em('Konvault\' LTC Wallet',lines.join('\n'),null,'balance')]});
