...
function rollPrize() {
  const total = PRIZES.reduce((a,b)=>a+b.c,0);
  const roll = hmacRoll(1,total); let c=0;
  for (const p of PRIZES) { c+=p.c; if(roll<=c) return p; }
...
module.exports = {
  ...
  async execute(interaction, client) {
    ...
    const match=p1.s===p2.s&&p2.s===p3.s, result=match&&p1.t>0?'WIN':'LOSS';
    let line;
    ...
    await log(client, { user: interaction.user, game: 'Scratch Card', result, detail: p1.s+' '+p2.s+' '+p3.s });
  },
};
