...
module.exports = {
  ...
  async execute(interaction) {
    ...
    await interaction.editReply({ embeds: [new EmbedBuilder().setColor(PURPLE).setDescription('🎡  Spinning...').setThumbnail(IMAGES.roulette).setFooter({ text: 'KONVAULT™', iconURL: IMAGES.logo })] });
    await wait(900);
    ...
    await interaction.editReply({
      embeds: [new EmbedBuilder()
        ...
        .setTitle('🎡  Roulette')
        .setThumbnail(IMAGES.roulette)
        .setDescription('**' + interaction.user.displayName + '** spun the wheel')
        .addFields(
          { name: 'Pocket', value: CE[p.c] + ' **' + p.n + ' ' + p.c.toUpperCase() + '**', inline: true },
          { name: 'Bet', value: '**' + bet.toUpperCase() + '**', inline: true },
