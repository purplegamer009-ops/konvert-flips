...
module.exports = {
  ...
  async execute(interaction) {
    ...
    await interaction.editReply({ embeds: [em('Konvault\' Roulette', '🎡  Spinning...', null, 'roulette')] });
    await wait(1000);

    const fairRoll = generateFairRoll(0, BOARD.length - 1);
    const p = BOARD[fairRoll.result];
    let won = false;
    ...
    const proofId = storeProof(interaction.channelId, {
      id: Date.now() + '_roulette',
      game: 'Roulette',
      result: CE[p.c] + ' ' + p.n + ' ' + p.c + ' — Bet: ' + bet + ' — ' + (won ? 'WIN' : 'LOSS'),
    ...
    await interaction.editReply({
      embeds: [em('Konvault\' Roulette', CE[p.c]+'  **'+p.n+'  '+p.c.toUpperCase()+'**\n\n'+(won?'✅  **'+interaction.user.displayName+'** wins':'❌  **'+interaction.user.displayName+'** loses'), null, 'roulette')],
