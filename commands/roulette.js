...
module.exports = {
  ...
  async execute(interaction, client) {
    ...
    const p = BOARD[rnd(0, BOARD.length-1)];
    let won = false;
    switch(bet) {
      case 'red':   won=p.c==='red';   break;
      case 'black': won=p.c==='black'; break;
      case 'green': won=p.c==='green'; break;
      case 'odd':   won=p.n>0&&p.n%2!==0; break;
      case 'even':  won=p.n>0&&p.n%2===0; break;
      case 'low':   won=p.n>=1&&p.n<=18; break;
      case 'high':  won=p.n>=19; break;
    }

    await interaction.editReply({ embeds: [em(
      'Konvert Flips\' Roulette',
    ...
    await log(client, {
      ...
      game: 'Roulette',
      result: won ? 'WIN' : 'LOSS',
      detail: `Bet: ${bet}  •  Result: ${CE[p.c]} ${p.n} ${p.c}`,
    });
  },
};
