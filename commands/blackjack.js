...
async function playHandDM(client,user,channel){
  ...
  try{dm=await user.createDM();}catch{await channel.send({embeds:[em('Konvault\' Blackjack','❌  Could not DM <@'+user.id+'>. Enable DMs.')]});return{total:0,bust:true,failed:true};}
  if(total(hand)===21){await dm.send({embeds:[em('Konvault\' Blackjack','🃏  **'+show(hand)+'**\n\n🎉  BLACKJACK!',null,'blackjack')]});await channel.send({embeds:[em('Konvault\' Blackjack','<@'+user.id+'> got their cards ✅')]});return{total:21,blackjack:true};}
  await dm.send({embeds:[em('Konvault\' Blackjack','Your hand:\n🃏  **'+show(hand)+'**  =  **'+total(hand)+'**\n\nType `hit` or `stand`',null,'blackjack')]});
...
module.exports = {
  ...
  async execute(interaction,client){
    ...
    await interaction.reply({content:`<@${opponent.id}>`,embeds:[em('Konvault\' Blackjack','<@'+interaction.user.id+'> challenged <@'+opponent.id+'> to **1v1 Blackjack!**\n\n<@'+opponent.id+'> — type `accept` or `decline`',null,'blackjack')]});
    let accepted=false;
    ...
    await interaction.channel.send({embeds:[em('Konvault\' Blackjack','🃏  Game on!\n\n<@'+interaction.user.id+'> goes first — check your DMs!',null,'blackjack')]});
    await wait(500);
    const r1=await playHandDM(client,interaction.user,interaction.channel);
    if(r1.failed)return;
    await wait(500);
    await interaction.channel.send({embeds:[em('Konvault\' Blackjack','<@'+interaction.user.id+'> done!\n\nNow <@'+opponent.id+'> — check your DMs!')]});
