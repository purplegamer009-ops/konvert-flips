...
module.exports = {
  ...
  async execute(interaction) {
    ...
    await interaction.editReply({ embeds: [new EmbedBuilder().setColor(PURPLE).setDescription('🃏  Drawing...').setThumbnail(IMAGES.highcard).setFooter({ text: 'KONVAULT™', iconURL: IMAGES.logo })] });
    await wait(900);
    ...
    await interaction.editReply({
      embeds: [new EmbedBuilder()
        ...
        .setDescription('**' + interaction.user.displayName + '** drew from the deck')
        .addFields({ name: 'Card', value: '**' + card + '**', inline: true })
