...
module.exports = {
  ...
  async execute(interaction) {
    ...
    await interaction.editReply({ embeds: [new EmbedBuilder().setColor(PURPLE).setDescription('🎲  Rolling...').setThumbnail(IMAGES.dice).setFooter({ text: 'KONVAULT™', iconURL: IMAGES.logo })] });
    await wait(900);
    ...
    await interaction.editReply({
      embeds: [new EmbedBuilder()
        ...
        .setDescription('**' + interaction.user.displayName + '** rolled the dice')
        .addFields(
          { name: 'Die 1', value: '**' + r1.result + '**', inline: true },
          { name: 'Die 2', value: '**' + r2.result + '**', inline: true },
          { name: 'Total', value: '**' + (r1.result + r2.result) + '**', inline: true },
        )
