...
module.exports = {
  data: new SlashCommandBuilder()
    .setName('highlow')
    .setDescription('🔢  1v1 Higher or Lower — take turns guessing the secret number!')
  ...
  async execute(interaction, client) {
    ...
    await interaction.reply({ embeds: [em('Konvert Flips\' Higher or Lower',
      '<@' + interaction.user.id + '> challenged <@' + opponent.id + '> to **Higher or Lower!**\n\n' +
      '🔢  Guess the secret number between **1 and 100**\n' +
      'Take turns — bot says higher or lower after each guess\n' +
    ...
    } catch { return interaction.channel.send({ embeds: [em('Konvert Flips\' Higher or Lower', '⏰  No response. Game cancelled.')] }); }
    if (!accepted) return interaction.channel.send({ embeds: [em('Konvert Flips\' Higher or Lower', '❌  <@' + opponent.id + '> declined.')] });
    const secret = hmacRoll(1, 100);
    const players = [interaction.user, opponent];
    let currentIndex = 0, guessCount = 0;
    await interaction.channel.send({ embeds: [em('Konvert Flips\' Higher or Lower', '🎮  Game on!\n\n🔢  Secret number locked in: **???**\n\n<@' + players[0].id + '> goes first — click the button below!')] });
    await wait(500);
    while (true) {
      const currentPlayer = players[currentIndex % 2];
      guessCount++;
      const guessRow = new ActionRowBuilder().addComponents(
        new ButtonBuilder().setCustomId('hilo_guess_' + currentPlayer.id + '_' + guessCount).setLabel('🔢  Enter Your Guess').setStyle(ButtonStyle.Primary)
      );
      const turnMsg = await interaction.channel.send({
        embeds: [em('Konvert Flips\' Higher or Lower', '<@' + currentPlayer.id + '> — your turn!\nClick the button to enter your guess secretly.')],
        components: [guessRow],
      });
      const guess = await new Promise(resolve => {
        ...
        col.on('collect', async btn => {
          ...
          try {
            const submitted = await btn.awaitModalSubmit({ time: 60000, filter: i => i.user.id === currentPlayer.id });
            const val = parseInt(submitted.fields.getTextInputValue('guess_value'));
            if (isNaN(val) || val < 1 || val > 100) { await submitted.reply({ content: '❌  Must be 1–100.', ephemeral: true }); clearTimeout(timeout); return resolve(null); }
            await submitted.reply({ content: '✅  **' + val + '** submitted!', ephemeral: true });
      ...
      if (!guess) return interaction.channel.send({ embeds: [em('Konvert Flips\' Higher or Lower', '⏰  <@' + currentPlayer.id + '> took too long. Game cancelled.')] });
      if (guess === secret) {
        const winner = currentPlayer;
        await interaction.channel.send({ embeds: [em('Konvert Flips\' Higher or Lower — Result',
          '<@' + currentPlayer.id + '> guessed **' + guess + '**\n\n✅  **CORRECT!**\n\n🏆  **<@' + winner.id + '> wins!**\n🔢  Secret number was **' + secret + '**\n📊  Total guesses: **' + guessCount + '**'
        )] });
      ...
      await interaction.channel.send({ embeds: [em('Konvert Flips\' Higher or Lower', '<@' + currentPlayer.id + '> guessed **' + guess + '**\n\n' + hint + '\n\n<@' + players[(currentIndex + 1) % 2].id + '> — your turn!')] });
      currentIndex++;
    }
  },
};
