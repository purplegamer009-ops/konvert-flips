...
module.exports = {
  data: new SlashCommandBuilder().setName('snipe').setDescription('🎯 1v1 Snipe — closest to the target without going over wins!')
  ...
  async execute(interaction,client){
    ...
    await interaction.reply({content:'<@'+opponent.id+'>',embeds:[new EmbedBuilder().setColor(PURPLE).setDescription('<@'+interaction.user.id+'> challenged <@'+opponent.id+'> to **Snipe**\n🎯 Both secretly pick 1–100\nClosest to the target without going over wins\n\n`accept` or `decline`').setThumbnail(IMAGES.verify).setFooter({text:'KONVAULT™',iconURL:IMAGES.logo})]});
    let accepted=false;
    ...
    let line,winner;
    if(!over1&&!over2){if(diff1<diff2){winner=interaction.user;line='🏆 **<@'+interaction.user.id+'> wins** — closer (**'+pick1+'** vs **'+pick2+'**)';}else if(diff2<diff1){winner=opponent;line='🏆 **<@'+opponent.id+'> wins** — closer (**'+pick2+'** vs **'+pick1+'**)';}else{line='🤝 TIE';}}
    else if(over1&&!over2){winner=opponent;line='🏆 **<@'+opponent.id+'> wins** — <@'+interaction.user.id+'> went over!';}
    else if(over2&&!over1){winner=interaction.user;line='🏆 **<@'+interaction.user.id+'> wins** — <@'+opponent.id+'> went over!';}
    else{const a1=Math.abs(pick1-target),a2=Math.abs(pick2-target);if(a1<a2){winner=interaction.user;line='🏆 **<@'+interaction.user.id+'> wins** — both over, closer';}else if(a2<a1){winner=opponent;line='🏆 **<@'+opponent.id+'> wins** — both over, closer';}else{line='🤝 TIE';}}
    const resultMsg=await interaction.channel.send({embeds:[new EmbedBuilder().setColor(winner?0x00E676:PURPLE)
      .setDescription('🎯 Target was **'+target+'**\n\n<@'+interaction.user.id+'> picked **'+pick1+'** '+(over1?'❌ over':'✅')+'\n<@'+opponent.id+'> picked **'+pick2+'** '+(over2?'❌ over':'✅')+'\n\n'+line)
      .setThumbnail(IMAGES.verify).setFooter({text:'KONVAULT™',iconURL:IMAGES.logo})]});
