...
module.exports = {
  ...
  async execute(interaction,client){
    ...
    const p1=interaction.options.getUser('player1'),p2=interaction.options.getUser('player2');
    ...
    try{
      ...
      await interaction.editReply({embeds:[new EmbedBuilder().setColor(PURPLE).setDescription('🎮 Channel locked to <@'+p1.id+'> and <@'+p2.id+'>\n\nEveryone else can watch. Use `/endgame` to unlock.').setFooter({text:'KONVAULT™',iconURL:IMAGES.logo})]});
    }catch(e){await interaction.editReply({content:'❌ Failed. Make sure bot has Manage Channel permission.'});}
  },
};
