...
module.exports = {
  ...
  async execute(interaction,client){
    ...
    await interaction.channel.send({embeds:[new EmbedBuilder().setColor(PURPLE)
      .setTitle('🔫  Russian Roulette')
      .setThumbnail(IMAGES.russian)
      .setDescription('🎲 Order determined randomly')
      .addFields({name:'1st',value:'<@'+players[0].id+'>',inline:true},{name:'2nd',value:'<@'+players[1].id+'>',inline:true})
      .setFooter({text:'KONVAULT™',iconURL:IMAGES.logo})]});
    await wait(1500);
    let round=1;
    while(true){
      for(const player of players){
        await interaction.channel.send({embeds:[new EmbedBuilder().setColor(PURPLE)
          .setDescription('**Round '+round+'** — <@'+player.id+'> raises the gun... 🔫')
          .setFooter({text:'KONVAULT™',iconURL:IMAGES.logo})]});
        await wait(1500);
        if(hmacRoll(1,6)===1){
          ...
          const resultMsg=await interaction.channel.send({embeds:[new EmbedBuilder().setColor(0x00E676)
            ...
            .setDescription('# 💥  BANG!')
            .addFields({name:'Eliminated',value:'<@'+player.id+'>',inline:true},{name:'Winner',value:'<@'+winner.id+'>',inline:true},{name:'Round',value:''+round,inline:true})
        ...
        await interaction.channel.send({embeds:[new EmbedBuilder().setColor(PURPLE)
          .setDescription('**Round '+round+'** — 🔔 CLICK — Empty chamber\n<@'+player.id+'> survived... passing the gun')
          .setFooter({text:'KONVAULT™',iconURL:IMAGES.logo})]});
        await wait(800);
      }
      round++;
