...
module.exports = {
  ...
  async execute(interaction, client) {
    ...
    await interaction.reply({ embeds: [em('Konvault\' RPS', '<@' + interaction.user.id + '> vs <@' + opponent.id + '>\n\n📩  **Both players check your DMs!**\nType your move there — nobody can see it.')] });
    const picks = {};
    async function getPick(user) {
      try {
        const dm = await user.createDM();
        await dm.send({ embeds: [em('Konvault\' RPS', 'Type your move:\n`rock`  `paper`  `scissors`\n\nYou have 60 seconds.')] });
      } catch { return; }
    ...
    if (!p1 || !p2) return interaction.channel.send({ embeds: [em('Konvault\' RPS', '⏰  Someone did not pick in time. Game cancelled.')] });
    let line, winner;
