...
module.exports = {
  ...
  async execute(interaction,client){
    ...
    const proofId = storeProof(interaction.channelId, {
      id: Date.now() + '_rps',
      game: 'RPS',
      result: interaction.user.username + ': ' + p1 + '  vs  ' + opponent.username + ': ' + p2 + ' — ' + (winner ? winner.username + ' wins' : 'TIE'),
    ...
    await interaction.channel.send({
      embeds:[em('Konvault\' RPS — Result','<@'+interaction.user.id+'>  '+E[p1]+'  **'+p1.toUpperCase()+'**\n<@'+opponent.id+'>  '+E[p2]+'  **'+p2.toUpperCase()+'**\n\n'+line,null,winner?'win':'rps')],
