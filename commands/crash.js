...
module.exports = {
  ...
  async execute(interaction, client) {
    ...
    await interaction.reply({ embeds:[em('Konvault\' Crash','<@'+interaction.user.id+'> challenged <@'+opponent.id+'> to **1v1 Crash!**\n\n<@'+opponent.id+'> — type `accept` or `decline`')] });
    let accepted=false;
    try {
    ...
    } catch { return interaction.channel.send({embeds:[em('Konvault\' Crash','⏰  No response. Game cancelled.')]}); }
    if (!accepted) return interaction.channel.send({embeds:[em('Konvault\' Crash','❌  <@'+opponent.id+'> declined.')]});
    await interaction.channel.send({embeds:[em('Konvault\' Crash','✅  Game accepted!\n\nBoth players — click your button to secretly enter your cashout.')]});
    ...
    if(!c1||!c2) return interaction.channel.send({embeds:[em('Konvault\' Crash','⏰  Someone did not set a cashout. Game cancelled.')]});
