...
module.exports = {
  ...
  async execute(interaction,client){
    ...
    await interaction.reply({content:`<@${opponent.id}>`,embeds:[em('Konvault\' Crash','<@'+interaction.user.id+'> challenged <@'+opponent.id+'> to **1v1 Crash!**\n\n<@'+opponent.id+'> — type `accept` or `decline`',null,'crash')]});
    let accepted=false;
    ...
    catch{return interaction.channel.send({embeds:[em('Konvault\' Crash','⏰  No response. Cancelled.')]});}
    if(!accepted)return interaction.channel.send({embeds:[em('Konvault\' Crash','❌  <@'+opponent.id+'> declined.')]});
    await interaction.channel.send({embeds:[em('Konvault\' Crash','✅  Accepted!\n\nBoth players click your button to set your secret cashout.')]});
    ...
    async function collectModal(msg,userId,customId){
      return new Promise(resolve=>{
        ...
        col.on('collect',async btn=>{
          ...
          try{const submitted=await btn.awaitModalSubmit({time:60000,filter:i=>i.user.id===userId});const val=parseFloat(submitted.fields.getTextInputValue('cashout_value'));if(isNaN(val)||val<1.01||val>100){await submitted.reply({content:'❌  Must be 1.01–100.',ephemeral:true});clearTimeout(timeout);return resolve(null);}await submitted.reply({content:'✅  **'+val+'x** locked! Waiting...',ephemeral:true});clearTimeout(timeout);resolve(val);}
    ...
    if(!c1||!c2)return interaction.channel.send({embeds:[em('Konvault\' Crash','⏰  Someone did not set a cashout. Cancelled.')]});
    await interaction.channel.send({embeds:[em('Konvault\' Crash','Both locked in!\n\n<@'+interaction.user.id+'>  **???x**\n<@'+opponent.id+'>  **???x**\n\n📈  Launching...',null,'crash')]});
    await wait(2000);
    ...
    await interaction.channel.send({embeds:[em('Konvault\' Crash — Result','💥  Crashed at  **'+cp+'x**\n\n<@'+interaction.user.id+'>  **'+c1+'x**  '+(p1s?'✅':'❌')+'\n<@'+opponent.id+'>  **'+c2+'x**  '+(p2s?'✅':'❌')+'\n\n'+line,null,winner?'win':'loss')]});
