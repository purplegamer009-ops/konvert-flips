...
// Stats — loaded from commands/stats.js which loads from disk on require
...
// Backup stats to Discord channel
async function saveStatsBackup() {
  try {
    ...
    await channel.send({ content: '📊 ' + new Date().toUTCString(), files: [attachment] });
    console.log('[Stats] Backup saved to Discord');
  } catch(e) { console.error('[Stats] Backup error:', e.message); }
}

// Restore stats from Discord backup on startup
async function loadStatsBackup() {
  try {
    ...
    if (!backupMsg) { console.log('[Stats] No Discord backup found'); return; }
    const url = backupMsg.attachments.first().url;
    const res = await fetch(url);
    const json = await res.text();
    importJSON(json);
    console.log('[Stats] Loaded from Discord backup');
...
client.on('messageCreate', async function(msg) {
  ...
  // ── Defeats tracker ─────────────────────────────────────────────────────────
  // Only owner, only in specific channels
  // Format: @winner defeats @loser NvN  (anything after NvN ignored)
  // Second number = amount at stake (winner gets +N, loser gets -N)
  if (msg.author.id === process.env.OWNER_ID && DEFEATS_CHANNELS.includes(msg.channelId)) {
    ...
    if (m) {
      const winnerId = m[1];
      const loserId  = m[2];
      const amount   = parseFloat(m[4]); // second number = stake
      ...
      // DM the owner privately — no public message
      try {
      ...
      // Save backup after every update
